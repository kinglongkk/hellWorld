...
var DdzDlgCardOp = DlgBase.extend({
	...
	init: function() {
		...
		this.Button_T = this._rootWidget.getChildByName("Button_T");
		this.Button_T.addTouchEventListener(function (sender, type) {
			if (ccui.Widget.TOUCH_ENDED == type) {
				if (!this.ResultSearchData) return;
				// 如果没有可出的牌，直接pass
				if (this.ResultSearchData.cbSearchCount <= 0) {
		...
		this.Button_T.setVisible(false);
		// 提示语
