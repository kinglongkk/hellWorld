...
var ZpmjGame = cc.Class.extend({
	...
	lTurnScore:[],					//局积分信息 （没用到，不做保存）
	lCollectScore:[],				//总积分信息 （没用到，不做保存）
	cbPlayerCount:0,	
	cbMaCount:0,	
	dwPlayCount:0,					//已玩局数
    flowerRecords:[0,0,0,0],					//各玩家的补花个数
    insertFlowerRecords:[0,0,0,0],			//各玩家的抓花个数
    ZhuaHuaCnt:0,						// 房间抓花数
    tingOutCards:[],					//玩家出牌能听的列表；只是针对自己才有
    cbHearStatus:[0,0,0,0],// 听牌的状态
    bTrustee:[false,false,false,false],// 托管状态
	...
	reset: function(){
		...
		this.cbHearStatus=[0,0,0,0];	//听牌状态0没听 1-3听
		...
		this.bTrustee = [false,false,false,false]; // 托管
		...
		this.gameStatus = CMD_ZPMJ.GS_MJ_FREE;
		this.cbMagicIndex = CMD_ZPMJ.MAX_INDEX;
		...
		this.lSiceCount = 0;
		//玩家牌型初始化
		this.playerCards = new Array(CMD_ZPMJ.GAME_PLAYER);
		for(var i=0;i<CMD_ZPMJ.GAME_PLAYER;i++)
		{
			this.playerCards[i] = new Array(CMD_ZPMJ.MAX_COUNT);
		...
		this.wHeapHead = 0;
		this.wHeapTail = 0;		
		this.cbHeapCardInfo = new Array(CMD_ZPMJ.GAME_PLAYER);
		for(var i=0;i<CMD_ZPMJ.GAME_PLAYER;i++)
		{
		...
		// 动作数据
		this.cbActionCard = CMD_ZPMJ.WIK_NULL;
		this.cbActionMask = CMD_ZPMJ.WIK_NULL;
		...
		// 玩家的补花个数
        this.flowerRecords = [0,0,0,0];

		// 玩家的插花个数
        this.insertFlowerRecords = [0,0,0,0];
		//---------------------//
	},
	//
    ...
    // 设置抓花数
    setZhuaHuaCnt:function(ZhuaHuaCnt)
	{
		this.ZhuaHuaCnt = ZhuaHuaCnt;
	},
	getZhuaHuaCnt:function () {
		return this.ZhuaHuaCnt;
    },
	...
	// 房间创建时间
    setRoomCreateTime:function(roomCreateTime){
        this.roomCreateTime = roomCreateTime;
    },
    getRoomCreateTime:function(){
    ...
    // 操作详细     主要用来表示扑克的方向 （可设，可不设，但是设有必要，其他用户就不用再去判断）
    setActionDetail:function(detail){
	...
	/*getMeUserInf:function(){
		...
		if(this.meChairId>=CMD_ZPMJ.GAME_PLAYER) return null;
	...
	setHisData:function(chairId,lTurnScore,lCollectScore){
		...
		if(chairId>=CMD_ZPMJ.GAME_PLAYER) return;
		var bFind=false;
	...
	/*	switchViewChairID:function(wChairID){
		...
		// var wChairCount = CMD_ZPMJ.GAME_PLAYER;
		// var wViewChairID =
	...
	setLeftCardCount:function(cbLeftCardCount){
		...
		cc.log("=======================================================>>>>>setLeftCardCount:"+cbLeftCardCount);
	...
	getLeftCardCount:function(){
		...
		cc.log("this.cbLeftCardCount:"+this.cbLeftCardCount);
    ...
    //听牌操作用户
    setListenOperateUser:function(wListenUser){
        this.ListenOperateUser=wListenUser;
    },

    getListenOperateUser:function(){
        return this.ListenOperateUser;
    },
	...
	setReplaceCard:function(cbReplaceCard){
		this.rePlaceCard = cbReplaceCard; // 花牌
	},
	getReplaceCard:function(){
		return this.rePlaceCard;
	},

    setReplaceNewCard:function(newCard){
        this.replaceNewCard = newCard; // 新牌
    },
    getReplaceNewCard:function(){
        return  this.replaceNewCard; // 新牌
    },

    setIsInisFlower:function(isInitFlower){
        this.isInitFlower = isInitFlower; // 新牌
    },
    getIsInisFlower:function(){
        return  this.isInitFlower; // 新牌
    },

    // 补花个数 记录
	setFlowerRecords:function (flowerRecords) {
        this.flowerRecords = flowerRecords;
    },
    getFlowerRecords:function() {
       return  this.flowerRecords;
	},


    getReplaceCardNumb:function(){
        return this.rePlaceCard;
    },

    // 插花个数 记录
    setInsertFlowerRecords:function (insertFlowerRecords) {
        this.insertFlowerRecords = insertFlowerRecords;
    },

    getInsertFlowerRecords:function() {
        return  this.insertFlowerRecords;
    },

	// 当前禁止出牌的卡牌值 比如吃了某张牌，不能打某张牌 存放到某个数组中，这些牌要变灰
	setDisableOutCards:function (BanOutCard ) {
		this.disableOutCards = BanOutCard;
    },
    getDisableOutCards:function () {
    	if(this.disableOutCards == undefined) {
            this.disableOutCards = [];
		}
        return this.disableOutCards;
    },

    // 出牌能听 的卡牌数组
    setTingOutCards:function (tingOutCards) {
		this.tingOutCards = tingOutCards;
    },

	getTingOutCards:function () {
        return this.tingOutCards;
    },
	...
	setPlayersCards:function(playersCards){
		...
		// 做一个清空处理
		for (var i = 0;i<CMD_ZPMJ.GAME_PLAYER;i++) {
			for(var k = 0;k<CMD_ZPMJ.MAX_COUNT+1;k++) {
                this.playerCards[i][k] = 0 ;
            }
		}

		var cardLength = playersCards.length; // 解决防止给的数据是16张不是17张

		for(var j=0;j<CMD_ZPMJ.MAX_COUNT;j++) {
			if(j >= cardLength) {
				this.playerCards[0][j] = 0;
            }
            else {
		...
		{
			var card =  this.playerCards[0][i];

			var count = 0;

            for(var k = 0;k<CMD_ZPMJ.MAX_COUNT;k++)
			{
				if(k==i)
					continue;

				var cardCmp = this.playerCards[0][k];
				...
				if(count>4) {
					...
					if(card==0)
					{
                        cc.log("发的牌值可能存在问题了0的个数是"+count);
					}
					else
					{
                        cc.log("服务器发的牌值有问题！！！");
                        DlgTip.openSysTip("--- 服务器发的牌值有问题,同样的牌值超过了4个"+card);
	...
	getMeCards:function(){
		...
		var meCards=new Array(CMD_ZPMJ.MAX_COUNT);
		for(var j=0;j<CMD_ZPMJ.MAX_COUNT;j++)
	...
	getOtherCards:function(){
		var otherCards=new Array(CMD_ZPMJ.MAX_COUNT);
	...
	//-----------------------------//
	// ui 部分临时存
    setOnGameEndUI:function (bOnEndUI) {
		this.bOnEndUI = bOnEndUI;
    },
    getOnGameEndUI:function () {
        return this.bOnEndUI;
    },

    // 取得战绩数据
    setGameRecordData:function (data) {
        this._GameRecordData = data
    },
    getGameRecordData:function () {
        return this._GameRecordData
    },
