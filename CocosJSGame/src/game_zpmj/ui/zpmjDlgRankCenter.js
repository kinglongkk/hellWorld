...
cc.log("--------ID_DlgZpmjRankCenter = "  + ID_DlgZpmjRankCenter);
DLG_CREATOR[ID_DlgZpmjRankCenter] = function() {
	return new DlgZpmjRankCenter();
};

var DlgZpmjRankCenter = DlgBase.extend({
	//成员函数
	ctor: function(){	
	...
	init: function() {		
		...
		var json = ccs.load(res.zpmjDlgRankCenter_json);
	...
	show: function(){
		...
		{//总积分
			...
			if(i==g_objHero.getChairID()){
				if(totalScore<=0)
					SoundMgr.getInstance().playEffect("zpmj_rankLose", 0, false);
				else
					SoundMgr.getInstance().playEffect("zpmj_rankWin", 0, false);
			}
		}
	},
