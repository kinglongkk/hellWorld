...
cc.log("--------ID_DlgZpmjMain = "  + ID_DlgZpmjMain);
DLG_CREATOR[ID_DlgZpmjMain] = function() {
	return new DlgZpmjMain();
};

var DlgZpmjMain = DlgBase.extend({
	
	ctor: function(){	
		cc.log("---ID_DlgZpmjMain--- ctor");
	},
	onCreate: function() {
		cc.log("---ID_DlgZpmjMain--- onCreate");
		this.init();
		this.reset();
	},
	onClose: function() {
		cc.log("---ID_DlgZpmjMain--- close");
	},
	reset: function() {
	},
    // 设置艺术字大小
	...
	init: function() {		
		// 背景音乐
		SoundMgr.getInstance().playMusic("zpmj_bgMusic",0,true);
		...
		var json = ccs.load(res.zpmjDlgMain_json);
        ...
        var sendInsertFlowersReq = function (sender, type) {
            switch(type) {
                case ccui.Widget.TOUCH_ENDED: {
                    ...
                    ZpmjGameMsg.getInstance().sendInsetFlowerCount(num);
                    break;
                }
            }
        };
        ...
        if(roomSpecialInfo!=undefined)
        {
            ...
            if(roomSpecialInfo.zhuaHua!=undefined ){
                info.push(roomSpecialInfo.zhuaHua>0 ? "抓花:"+roomSpecialInfo.zhuaHua+"张":"不抓花");
			}
            if(roomSpecialInfo.wanFa!=undefined){
                info.push(roomSpecialInfo.wanFa==true ? "带大字":"不带大字");
            }
            if(roomSpecialInfo.suanFen!=undefined){
                info.push(roomSpecialInfo.suanFen==0 ? "33制" : (roomSpecialInfo.suanFen==1 ?  "48制" : "88制"));
            }
            if(roomSpecialInfo.chaHua!=undefined){
                info.push(roomSpecialInfo.chaHua ? "插花":"不插花");
            }
        ...
        dlgChatScene.BtnSendMsg.setPosition(cc.p(btnMsgPos.x+4,btnMsgPos.y+40));
        dlgChatScene.BtnSounds.setPosition(cc.p(btnSoundPos.x+4,btnSoundPos.y+20));
    ...
    hideTipQuit:function () {
        cc.log("hideTipQuit calling1");
        this.Panel_tip.setVisible(false);
		this.Panel_tip.unscheduleAllCallbacks();
        cc.log("hideTipQuit calling2");
    },
    showTip: function () {
        // 增加一个判断, 游戏中直接 关闭定时器
        ...
        cc.log("房间即将超时，请尽快开始游戏");
        this.quitTimeText.string =  "房间即将超时请尽快开始游戏（" + this.quitTime + "）";
        this.quitTime --;
        this.Panel_tip.setVisible(true);

        if(this.quitTime==undefined || this.quitTime <0) {
            cc.log("hideTipQuit calling3333");
            this.hideTipQuit();
        }
    },
    startShowTip:function () {
        this.Panel_tip.schedule(this.showTip.bind(this), 1, cc.REPEAT_FOREVER, this);
    },
	// 开启房间超时提示的定时器
    showTipQuit:function () {
        ...
        var table = ClientData.getInstance().getTable();
        if( CMD_ZPMJ.GS_MJ_PLAY == table.getGameStatus()) {
            cc.log("游戏已经开始了，直接返回");
            this.hideTipQuit();
            return;
        }
        ...
        var timestamp = ClientData.getInstance().getServerTime(); // 同步服务器时间，如果没有就会自动获取本地时间
        var off = timestamp - createTime;
        cc.log("createTime:"+createTime+"timestamp:"+timestamp+"已经偏移了"+off);

        var overTime = 30*60;
        // var overTime = 120; // 测试数据

        if(off >= overTime) { // 30分钟没结束去掉
            this.hideTipQuit();
            cc.log("时间超过30分钟了，房间已经结束");
            return;
        }
        var lastSec = (overTime-60 - off)>0 ? (overTime-60 - off) : (off - (overTime - 60));  // 29*60-off>0 ? 29*60-off : off - 29*60 ; // test
        if(lastSec<60)
            this.quitTime = lastSec;
        else
            this.quitTime = 60;

		// 直接在这个对象的 Panel_tip 的属性节点上开启定时器来控制 节点自身的显隐
		this.Panel_tip.schedule(this.startShowTip.bind(this), lastSec, false, this);
    },
...
//		]);
		...
		var index = 0
		while(true){ if(((meChairID+index)%CMD_ZPMJ.GAME_PLAYER)==chairID) break; index++; }
		var strWho = "My";
		switch(index){
				...
				return function(even){
                    ...
                    if(DlgZpmjMain)
					{
                        if(DlgZpmjMain.Text_gold){
                            ...
                            if(value!=undefined)
                                DlgZpmjMain.Text_gold.string = value;
                        }
					}
				}
			};
