var YxmjGameMsg = GameMsg.extend({
	ctor: function(){
        cc.log("---------- YxmjGameMsg. 消息中转器被建创--------------------")
        this.setName("YxmjGameMsg 消息中转器")
	},
	...
	isCurRunGame: function(){
		...
		if(plaza){
			...
			cc.log("云霄isCurRunGame====kindId"+kindId);
			if(kindId == CMD_YXMJ.KIND_ID){
				return true;
			}
		}
		return false;
	...
	onGameStatus: function(){
		...
		cc.log("云霄onGameStatus====111");
		if(!this.isCurRunGame()){
			return;
		}
		
		cc.log("云霄onGameStatus====333");
		
		// set game
		var game = new YxmjGame();
	...
	onGameMsg: function(subCmd, data){
		...
		switch (subCmd) {
		case CMD_YXMJ.SUB_S_GAME_START:		// 游戏开始
			 this.OnSubGameStart(data);
			cc.log("-------------------!!游戏开始");
			break;

        case CMD_YXMJ.SUB_S_DISPATCH_FLOWER:
        	this.OnSubDispatchFlower(data);
        	cc.log("-------------------!!开局or抓牌 ------补花");
        	break;

		case CMD_YXMJ.SUB_S_GOLD_MEDAL:
			this.OnSubGoldMedal(data);
			cc.log("-------------------!!翻金");
			break;

		case CMD_YXMJ.SUB_S_SEVEN_FLOWER:
			/*
		...
		case CMD_YXMJ.SUB_S_OUT_CARD:		// 用户出牌
			/*
			 * 说明 收到用户出牌
			 */
		...
		case CMD_YXMJ.SUB_S_SEND_CARD:		// 发牌消息
			/*
		...
		case CMD_YXMJ.SUB_S_OPERATE_NOTIFY:	// 操作提示
			/*
			 * 说明
		...
		case CMD_YXMJ.SUB_S_OPERATE_RESULT:	// 操作结果（吃碰杠胡）
			/*
			...
			case CMD_YXMJ.SUB_S_LISTEN: // 听牌的操作结果（听）
			cc.log("-------------------!!其他用户（包括自己）告诉我，他听牌 操作结果");
			this.OnSubListenOperat(data);
			break;

		case CMD_YXMJ.SUB_S_HU_CARD:
			cc.log("-------------------!!后台系统告诉我 听牌=====>>>>胡牌数据");
			this.OnTingHuCard(data);
			break;

		case CMD_YXMJ.SUB_S_TRUSTEE:
			cc.log("-------------------!!用户托管or取消托管 操作");
			this.onGameTrustee(data);
			break;
		case CMD_YXMJ.SUB_S_GAME_END:		// 游戏结束
			this.OnSubGameEnd(data);
			cc.log("-------------------!!游戏结束");
			break;		
		case CMD_YXMJ.SUB_S_CONFIG_CELLSCORE:  // 配置底注
			this.OnUserConfigCellScore(data);
			break;
		case CMD_YXMJ.SUB_S_MAKESURE:		// 用户坐下
			this.OnUserMakesure(data);
			cc.log("-------------------!!用户坐下");
			break;
		case CMD_YXMJ.SUB_S_USEREXIST:		// 用户退出
			this.OnUserExist(data);
			cc.log("!!用户退出");
			break;
		case CMD_YXMJ.SUB_S_USERCLOSEWIN:	// 关闭窗口
			this.OnUserExist(data);
			cc.log("!!关闭窗口");
			break;
		default:
    ...
    onGameSceneMsg: function(data, isPlay){
        ...
        cc.log("云霄麻将 游戏场景消息-gameStatus-"+gameStatus);
        ...
        if (!isPlay) {// 空闲状态
            cc.log("云霄麻将 游戏场景消息--空闲状态");
            cc.log("游戏场景消息 = " + JSON.stringify(data));
            // /////////////////////////////////先数据处理//////////////////////////////////////////
            table.setGameStatus(CMD_YXMJ.GS_MJ_FREE);
            ...
            //获取历史记录
            YxmjUIMgr.getInstance().getGameInfo();
            ...
            // 设定相关信息
            //游戏状态
            table.setGameStatus(CMD_YXMJ.GS_MJ_PLAY);
            //时间信息
            game.setTimeOutCard(data.TimeOutCard);
            game.setTimeOperateCard(data.TimeOperateCard);
            game.setTimeStartGame(data.TimeStartGame);
	    ...
	    for(var i = 0;i<data.BuHuaCnt.length;i++) {
            ...
            if(runScene && runScene.isGameScene && runScene.isGameScene()){
                cc.log("YxmjUIMgr.getInstance().onBreak()");
                YxmjUIMgr.getInstance().onBreak();
            }
        }
	...
	onSubNotifyShowInsertFlowerUI:function (data) {
        YxmjUIMgr.getInstance().onShowInserFlowerUI(true);
        cc.log("通知显示插花界面 ");
    },
    ...
    OnSubInsetFlowerCount:function (data) {
        ...
        if (parseData.Chair ==  game.getMeChairId()) {
            YxmjUIMgr.getInstance().onShowInserFlowerUI(false); // 隐藏插花界面
		}

        //  更新玩家插花个数
        var insertFlowerRecords = game.getInsertFlowerRecords();
        insertFlowerRecords[parseData.Chair] = parseData.SetCount;
        YxmjUIMgr.getInstance().updateInsertFlowerNum(parseData.Chair,false);
    },
	...
	OnSubGameStart:function(data){
		...
		*/
        ...
        game.setLeftCardCount(CMD_YXMJ.FULL_COUNT - (CMD_YXMJ.MAX_COUNT-1)*4 -1);// 得利用服务器的数据
		...
		// ///////////////////
		YxmjUIMgr.getInstance().onShowInserFlowerUI(false); // 游戏一开始 隐藏插花页面 没有插花的用户，就不在显示
		YxmjUIMgr.getInstance().onGameStart();
		
		// 更新历史记录
		YxmjUIMgr.getInstance().updateGameInfo();
	},
    ...
    OnSubDispatchFlower:function (data) {
		...
		*/
        ...
        if(
		flowerRecords!=undefined && 
		flowerRecords[parseData.ReplaceUser]!=undefined &&
        ...
        YxmjUIMgr.getInstance().replaceCard();  // 得区分是开局补花还是，抓牌补花
        if (parseData.ReplaceUser == game.getMeChairId())
        {
			if( parseData.NewCard>= g_Yxmj_Hua_CardValue[0] && parseData.NewCard<= g_Yxmj_Hua_CardValue[7])
			{
                // 可能还是花得再进行 补花
                cc.log("补花后 ========再进行补花 " + parseData.NewCard);
                var dataParam={
                    cbCardData:parseData.NewCard,
                };
                YxmjGameMsg.getInstance().sendReplaceCard(dataParam);
			}
			else
			{
                ...
                YxmjUIMgr.getInstance().onSendCard();
			}
		}
    },
    ...
    OnSevenFlower:function (data) {
    	...
    	YxmjUIMgr.getInstance().replaceCard_7Flower();
    ...
    OnSubGoldMedal:function (data) {
        ...
        var cbUpdateGold =  function () {
            ...
            YxmjUIMgr.getInstance().updateGoldMedal();
            game.setLeftCardCount( game.getLeftCardCount() - 1 );
        };
        ...
        if (parseData.FlowerCount) {
            ...
            YxmjUIMgr.getInstance().updateFloweNum(0);
            game.setLeftCardCount( game.getLeftCardCount() - parseData.FlowerCount);

            // 0.5秒后变真金牌
            YxmjUIMgr.getInstance().schedule(cbUpdateGold, 0.5, false, this);
		}
		else {
            cbUpdateGold();
		}
	...
	OnSubOutCard:function(data){
		...
		*/
        ...
        // 更新UI托管标志
        YxmjUIMgr.getInstance().onTrustee();

		// 界面显示
		YxmjUIMgr.getInstance().onOutCard();

	},

	// 后台发牌消息
	OnSubSendCard:function(data){
		cc.log("云霄麻将 游戏场景消息--发牌消息--");
		...
		*/
		...
		if (parseData.CurrentUser == game.getMeChairId() &&
			parseData.CardData>= g_Yxmj_Hua_CardValue[0] &&
			parseData.CardData<= g_Yxmj_Hua_CardValue[7])
		{
	...
	OnSubOperateResult:function(data){
		...
		*/
		...
		YxmjUIMgr.getInstance().onOperateResult();
	},
    ...
    OnSubListenOperat:function (data) {
		...
		*/
        ...
        YxmjUIMgr.getInstance().onListenOperateResult(); // 播听语音，并附上状态
    },
    ...
    OnTingHuCard:function (data) {
		...
		*/
		...
		game.setTingOutCards(parseData.OutCardData);
   		...
   		YxmjUIMgr.getInstance().onTingHuCard(); // 显示操作按钮
    },
	...
	OnSubGameEnd:function(data){
		...
		*/
		// test
		/*data = {"CellScore":0,"GameScore":[65535,64,65532,65538],"Revenue":[0,0,0,0],"GangScore":[0,0,0,0],
			"ProvideUser":1,"ProvideCard":20,"SendCardData":20,"ChiHuKind":[64,0,0,0],
			"ChiHuRight":[0,134219521,0,0],"LeftUser":65535,"LianZhuang":0,"CardCount":[7,13,13,7],
			"HandCardData":[[1,1,20,21,22,37,38,0,0,0,0,0,0,0],[4,5,6,17,18,19,20,21,21,22,23,23,24,0],[5,5,17,17,19,33,33,33,35,36,37,38,38,0],[7,18,23,24,24,24,34,0,0,0,0,0,0,0]],
			"MaCount":[0,0,0,0],"MaData":[0,0,0,0,0,0,0],
			"ZhuaHua":[{"ChairID":0,"Card":1,"IsZhong":1},
                {"ChairID":1,"Card":1,"IsZhong":true},
                {"ChairID":2,"Card":2,"IsZhong":false},
                {"ChairID":3,"Card":3,"IsZhong":false},
                {"ChairID":0,"Card":4,"IsZhong":true},
                {"ChairID":1,"Card":5,"IsZhong":false},
                {"ChairID":2,"Card":6,"IsZhong":true},
                {"ChairID":3,"Card":7,"IsZhong":false},
                {"ChairID":0,"Card":8,"IsZhong":true},
                {"ChairID":1,"Card":9,"IsZhong":false},
                {"ChairID":2,"Card":0x11,"IsZhong":true},
                {"ChairID":3,"Card":0x12,"IsZhong":true},
                {"ChairID":0,"Card":0x13,"IsZhong":false},
                {"ChairID":1,"Card":0x14,"IsZhong":true},
                {"ChairID":2,"Card":0x15,"IsZhong":true},
                {"ChairID":3,"Card":0x16,"IsZhong":true},
            ]}
*/
	...
	OnUserConfigCellScore:function(data){
		...
		var dlgUserInf=UIMgr.getInstance().getDlg(ID_DlgYxmjUserInf);
		...
		if(parseData.wConfigUser == game.getMeChairId())
		{
			...
			dataBuilder.init(0);
			g_gameSocket.sendData(MDM_GF_GAME,CMD_YXMJ.SUB_C_MAKESURE,dataBuilder.getData());
		}	
	},
...
var g_YxmjGameMsg = null;

YxmjGameMsg.getInstance = function(){
	if(g_YxmjGameMsg == null){
		g_YxmjGameMsg = new YxmjGameMsg();
	}
	return g_YxmjGameMsg;
}

GameMsgMgr.getInstance().addGameMsgInstance(YxmjGameMsg.getInstance());
