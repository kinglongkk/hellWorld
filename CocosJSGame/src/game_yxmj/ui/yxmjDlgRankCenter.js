...
cc.log("--------ID_DlgYxmjRankCenter = "  + ID_DlgYxmjRankCenter);
DLG_CREATOR[ID_DlgYxmjRankCenter] = function() {
	return new DlgYxmjRankCenter();
};

var DlgYxmjRankCenter = DlgBase.extend({
	//成员函数
	ctor: function(){
        cc.log(" ---------- 云霄麻将DlgYxmjRankCenter.ctor() ---------- ");
	},
	onCreate: function() {
		this.init();
		this.reset();
	...
	init: function() {		
		...
		var json = ccs.load(res.yxmjDlgRankCenter_json);
	...
	show: function(){
		...
		{//总积分
			...
			if(i==g_objHero.getChairID()){
				if(totalScore<=0)
					SoundMgr.getInstance().playEffect("yxmj_rankLose", 0, false);
				else
					SoundMgr.getInstance().playEffect("yxmj_rankWin", 0, false);
			}
		}
	},
	...
	onClickEvent: function(sender, type){
		if (ccui.Widget.TOUCH_ENDED == type){	
			...
			case "Button_quit":
				cc.log("++++++战绩中心界面----返回++++++");
				//关闭战绩中心
				UIMgr.getInstance().closeDlg(ID_DlgYxmjRankCenter);
...
//				//关闭战绩中心
//				UIMgr.getInstance().closeDlg(ID_DlgZpmjRankCenter);
//
