...
var YxmjUIMgr = GameUIMgr.extend({
	reset: function () {
		cc.log("YxmjUIMgr------reset--");
        cc.log(" ---------- 云霄麻将YxmjScene.ctor() ---------- ");
	},

	startGame: function () {
		cc.log("YxmjUIMgr------startGame--");

		this.reset();
		//设置游戏人数
		this.setPlayerCount(CMD_YXMJ.GAME_PLAYER);

		var sizeDir = cc.director.getWinSize();

		//创建和打开牌面
        var dlg = UIMgr.getInstance().openDlg(ID_DlgYxmjCardsInfo);
		
		//创建和打开主界面
		UIMgr.getInstance().openDlg(ID_DlgYxmjMain);
		...
		if(table.getGameStatus() == CMD_YXMJ.GS_MJ_PLAY)
			this.onBreak();
	...
	onUpdateAllPlayerInfo: function(){	
		...
		for(chairID=0; chairID<CMD_YXMJ.GAME_PLAYER; ++chairID){
			if(!DlgYxmjMain.setUserInfo(chairID)){
				isPlayersFull = false;
			}
		}
		...
		// 更新按钮
        if(isPlayersFull){
            DlgYxmjMain.Panel_waitOperation.setVisible(false);
        }
        else{

            DlgYxmjMain.Panel_waitOperation.setVisible(true);
            DlgYxmjMain.Button_invite.setVisible(true);
            ...
            if(player.getStatus()==US_READY) {
                DlgYxmjMain.Button_invite.setPositionX(400);
                DlgYxmjMain.Button_startGame2.setVisible(false);
	    }
	    else {
	        DlgYxmjMain.Button_startGame2.setVisible(true);
		DlgYxmjMain.Button_startGame.setVisible(false);
	    }
            DlgYxmjMain.Button_startGame.setVisible(false);
        }
    ...
    onShowInserFlowerUI:function (bShow) {
        ...
        var dlg = UIMgr.getInstance().getDlg(ID_DlgYxmjMain);
        if (!dlg) return;
                ...
                dlg.scheduleChaHuaCb = function () {
                    ...
                    if (dlgMain!=undefined) {
                        if(dlgMain.charHuaTimes==undefined || dlgMain.charHuaTimes <0) {
                            YxmjUIMgr.getInstance().unschedule(dlgMain.scheduleChaHuaCb, dlgMain);
    ...
    onTingHuCard: function(){
        ...
        if(!dlgCardsInf) return;
        var arrOperator = [];
        arrOperator.push(CMD_YXMJ.WIK_LISTEN);
	...
	onOperatorTip:function(){
		...
		//吃
        if(cbActionMask & CMD_YXMJ.WIK_LEFT || cbActionMask & CMD_YXMJ.WIK_CENTER || cbActionMask & CMD_YXMJ.WIK_RIGHT) {
            arrOperator.push(CMD_YXMJ.WIK_LEFT);// 代表吃 用来显示吃按钮
        }
		//碰牌类型
		if(cbActionMask & CMD_YXMJ.WIK_PENG) {
			arrOperator.push(CMD_YXMJ.WIK_PENG);
		}
		//杠牌类型
		if(cbActionMask & CMD_YXMJ.WIK_GANG) {
			arrOperator.push(CMD_YXMJ.WIK_GANG);
		}
		//胡类型
		if(cbActionMask & CMD_YXMJ.WIK_CHI_HU) {
			arrOperator.push(CMD_YXMJ.WIK_CHI_HU);
		}
		if(cbActionMask & CMD_YXMJ.WIK_FANG_PAO) {
			arrOperator.push(CMD_YXMJ.WIK_FANG_PAO);
		}
	...
	onGameStart: function(){
		...
		var DlgYxmjMain = UIMgr.getInstance().getDlg(ID_DlgYxmjMain);
		if(!DlgYxmjMain) return;

        DlgYxmjMain.hideTipQuit();// 取消提示
        DlgYxmjMain.Panel_waitOperation.setVisible(false);
		
		SoundMgr.getInstance().playEffect("yxmj_gameStart", 0, false);
	},
	//断线重连
	onBreak:function(){
		var dlgCardsInf=UIMgr.getInstance().getDlg(ID_DlgYxmjCardsInfo);
	...
	updateGameInfo: function(){
		...
		//保存
		LocalStorageMgr.getInstance().setYxmjGameInfo(strGameInfo);
	},
	//获取本地游戏记录--进入游戏的时候获取记录 并保存到game model 
	getGameInfo: function(){
		var strGameInfo = LocalStorageMgr.getInstance().getYxmjGameInfo();
	...
	removeGameInfo: function(){
		LocalStorageMgr.getInstance().setYxmjGameInfo("");
	},
...
var g_YxmjUIMgr = null;

YxmjUIMgr.getInstance = function() {
	if (!g_YxmjUIMgr) {
		g_YxmjUIMgr = new YxmjUIMgr();
	}
	return g_YxmjUIMgr;
};