...
cc.log("--------ID_DlgYxmjCardsInfo = "  + ID_DlgYxmjCardsInfo);
DLG_CREATOR[ID_DlgYxmjCardsInfo] = function() {
    return new DlgYxmjCardsInfo();
};



var DlgYxmjCardsInfo = DlgBase.extend({
    //成员函数
    ctor: function(){
        cc.log(" ---------- 云霄麻将DlgYxmjCardsInfo.ctor() ---------- ");
    },
    onCreate: function() {
        this.init();
        this.reset();
    ...
    // 初始化
    init: function() {
        var json = ccs.load(res.yxmjDlgCardInfo_json);
        ...
        var posit;
        this.imgFOtherCard = [];
        for(var chairID=0;chairID<CMD_YXMJ.GAME_PLAYER; chairID++){
            var curChairID = (meChairID+chairID)%CMD_YXMJ.GAME_PLAYER;
            switch(chairID){
                case 0:{
                    //自己
                    this.imgFUserCard=[];
                ...
                case 1:{
                    ...
                    for(var i=CMD_YXMJ.MAX_COUNT-1;i>=0; i--){
        ...
        this.opFrameCach.addSpriteFrames(res.yxmjAnimate_plist);
    ...
    getUserUIIndex:function (user) {
        ...
        var index = 0;
        while(true){
            if(((meChairID+index)%CMD_YXMJ.GAME_PLAYER)==user){
                break;
            }
            index++;
        }
    ...
    getUserUIByUserIndex:function (user1,user2) {
        ...
        var index = 0;
        while(true){
        	if(((user1+index)%CMD_YXMJ.GAME_PLAYER)==user2){
                break;
            }
            index++;
        }
    ...
    replaceCard:function() {
        ...
        if (replaceUser == meChairID) {
            ...
            for (var i=0;i<CMD_YXMJ.MAX_COUNT;i++){
                ...
                if (oldCardValue == this.cardData[i]) // 更新牌值：判断是否是开局补花，开局的就得排序，不是开局是打牌的就不用
                {
                    ...
                    if(game.getIsInisFlower()==true) {
                        this.updateCard(this.cardData, CMD_YXMJ.MAX_COUNT); // 是否做排序，如果是 抓牌位 张就不做排序
                        cc.log("开局补花，更新所有牌");
                    }
                    else {
            ...
            if (bfound = false) {
                ...
                this.cardData[ CMD_YXMJ.MAX_COUNT-1] = newCardValue;
                this.displayCard(this.cardData[ CMD_YXMJ.MAX_COUNT-1],  CMD_YXMJ.MAX_COUNT-1); // 抓牌就是最后一张
        ...
        SoundMgr.getInstance().playEffect("yxmj_"+ strSex +"replace", 0, false);
    },
    ...
    replaceCard_7Flower:function () {
        ...
        {
            this.cardData[CMD_YXMJ.MAX_COUNT-1] = 100; // 隐藏
            this.imgFUserCard[CMD_YXMJ.MAX_COUNT-1].setVisible(false);
        }
        else
        {
            this.imgFOtherCard[oldUser][CMD_YXMJ.MAX_COUNT-1].setVisible(false);
        }
    ...
    updateGoldMedal:function() {
        ...
        if (cardValue != undefined)
        {
        	...
        	this.updateCard(this.cardData,CMD_YXMJ.MAX_COUNT);
       	}
    },
    ...
    */
    ...
    doClear: function(){
        ...
        if(DlgYxmjMain){
            DlgYxmjMain.Panel_OnTopTouch.setVisible(false);
        }
        var game = ClientData.getInstance().getGame();
        if(game)
    ...
    updateTrustee:function () {
       ...
       // 托管的话，只关注自己；1：:手牌要变灰；
        ...
        DlgYxmjMain.Panel_OnTopTouch.setVisible(trustee[myChildId]);
        // 手牌是否变灰色 待续
    ...
    updateAll13CardPossist:function () {
        for (var i=0;i<CMD_YXMJ.GAME_PLAYER;i++) {
    ...
    setThe13CardPosist:function (currentUser) {
        ...
        for(var i = 0;i<CMD_YXMJ.MAX_COUNT-1;i++) {
        ...
        Temp[CMD_YXMJ.MAX_COUNT-1].setPosition(cc.p( x,y));
    },

    //后台发牌消息 （也有动作掩码，判断 自摸和杠  也需要告知是否有显示操作面板 ）
    sendCard:function() {
        cc.log("----yxmjDlgCardInfo.sendCard----");
        var game=ClientData.getInstance().getGame();
        ...
        //显示发牌者的 第13张手牌
        if(currentUser == myChair) {
            this.cardData[CMD_YXMJ.MAX_COUNT-1]=value;
            this.displayCard(value,CMD_YXMJ.MAX_COUNT-1);
        }
        else {
            this.imgFOtherCard[currentUser][CMD_YXMJ.MAX_COUNT-1].setVisible(true);
        }
        ...
        if (currentUser == myChair)
        {
            ...
            //自摸
            if(actionMask & CMD_YXMJ.WIK_CHI_HU) {
                cc.log("自摸");
                this.zimo=this.cardData[CMD_YXMJ.MAX_COUNT-1];
                arrOperator.push(CMD_YXMJ.WIK_CHI_HU);
            }
            var countGang=0;			//计算杠的数量
            var count = CMD_YXMJ.MAX_COUNT-2;				//和摸的牌相比的牌
            ...
            for(var i=0;i<count;i++)
            {
                ...
                if(i<count-2) {
                        ...
                        this.cardData[i] == this.cardData[i+2])
                    {
                        ...
                        countGang++;
                        i = i+3;
                        var obj = {"type":CMD_YXMJ.TYPE_GANG_HAND4 , "value":this.cardData[i]};
            ...
            if(actionMask & CMD_YXMJ.WIK_GANG)
            {
                //分析明杠 暗杠
                //1 如果原来有碰  明杠
                ...
                for(var i=0; i<this.openFCardData.length;i++)
                {
                    ...
                    if(openCard.cardMask&CMD_YXMJ.WIK_PENG)
		    {
                    	...
                    	if (openCard.cardValue == value) // 抓的牌就是要杠的牌，开牌碰有3张
                        {
                            ...
                            countGang++;
                            var obj = {"type":CMD_YXMJ.TYPE_GANG_PENG , "value":value};
                            this.handGangs.push(obj);
                            break;
                        }
                    	else // 抓的牌不是要杠的牌，而是手牌其中的一张牌，可以和开牌的碰组成杠
                        {
                            ...
                            for(var j=0;j<CMD_YXMJ.MAX_COUNT;j++)
                            {
                                ...
                                if(openCard.cardValue != this.cardData[j]){
                                    ...
                                    countGang++;
		                    var obj = {"type":CMD_YXMJ.TYPE_GANG_PENG , "value":openCard.cardValue};
		                    this.handGangs.push(obj);
		                    break;
                                }
                            }
                ...
                for(var i=0;i<CMD_YXMJ.MAX_COUNT-3;i++){
                    ...
                    if(this.cardData[i]==this.cardData[i+1] && this.cardData[i]==this.cardData[i+2])
		    {
                        ...
                        var obj = {"type":CMD_YXMJ.TYPE_GANG_HAND3_MY , "value":value};
                        this.handGangs.push(obj);
                        break;
                    }
                }
            ...
            if(countGang!=0){
                //显示操作面板
                arrOperator.push(CMD_YXMJ.WIK_GANG);
            }
    ...
    //用户出牌
    outCard:function() {
        cc.log("----yxmjDlgCardInfo.outCard----");
        var game=ClientData.getInstance().getGame();
        ...
        //报牌声音播放
        var strSoundFile = "yxmj_"+strSex+strCard;
        ...
        if(myChair != wOutCardUser) {
            cc.log("wOutCardUser+++++++++"+wOutCardUser);
            //隐藏出牌玩家的第13张 手牌
            this.imgFOtherCard[wOutCardUser][CMD_YXMJ.MAX_COUNT-1].setVisible(false);
            // 显示出牌
    ...
    onListenOperateResult:function() {
        ...
        if (operateUser==mychairid) {
            ...
            */
            ...
            var state = DlgYxmjMain.Panel_ready.getChildByName(readyStateNodeName);
            state.setVisible(true);
            var imagePath = "yxmj_main/main_listernState.png";
        ...
        SoundMgr.getInstance().playEffect("yxmj_"+strSex+"_ting", sex, false);
    },
    ...
    setGameClock: function(nextUser,times){
        ...
        if(this.scheduleCallBack)
            YxmjUIMgr.getInstance().unschedule(this.scheduleCallBack, this);
        ...
        var texture = "";
        if(index==0) {
            texture =  "yxmj_main/main_round_pointer_down.png";
        } else if(index==1) {
            texture =  "yxmj_main/main_round_pointer_right.png";
        } else if(index==2) {
            texture =  "yxmj_main/main_round_pointer_up.png";
        } else if(index==3) {
            texture =  "yxmj_main/main_round_pointer_left.png";
        }
       ...
       // cc.log("---this.clockActionUser--"+this.clockActionUser+"---倒数时间---------------------"+times);
        ...
        YxmjUIMgr.getInstance().schedule(this.scheduleCallBack, 1, cc.REPEAT_FOREVER, this);
    },
...
//		this.Clock.setVisible(false);
    ...
    getFCard:function(cardVal){
        ...
        cc.log("牌    "+"yxmj_cardAndColor/"+color[nCardColor]+"/"+color[nCardColor]+"_"+nValue+".png");
        return ("yxmj_cardAndColor/"+color[nCardColor]+"/"+color[nCardColor]+"_"+nValue+".png");
    },
        ...
        else
        {
            if(this.imgFUserCard[pos]!=null)
            {
                ...
                if(colorImage!=null )
                {
                    ...
                    // 云霄麻将有金,需要给每张手牌判断是否是金牌,如果是金， 则显示动画；
    ...
    showCard: function(){
        ...
        this.updateCard(this.cardData,CMD_YXMJ.MAX_COUNT);   //更新牌
        ...
        //显示其他玩家的固定手牌
        for(var userID=0; userID<CMD_YXMJ.GAME_PLAYER; userID++){
        ...
        for(var chairID=0;chairID<CMD_YXMJ.GAME_PLAYER; chairID++){
            if(bankerID==chairID){
                (chairID==meChairID)?this.imgFUserCard[CMD_YXMJ.MAX_COUNT-1].setVisible(true):this.imgFOtherCard[chairID][CMD_YXMJ.MAX_COUNT-1].setVisible(true);
            }
            else{
                (chairID==meChairID)?this.imgFUserCard[CMD_YXMJ.MAX_COUNT-1].setVisible(false):this.imgFOtherCard[chairID][CMD_YXMJ.MAX_COUNT-1].setVisible(false);
            }
        }
    ...
    onGameEnd:function(resultData){
        ...
        var gameEndFun = function () {
        	var dlg=UIMgr.getInstance().getDlg(ID_DlgYxmjCardsInfo);
            var showGameEndDlgs = function (result) {
                ...
                if(curRounds==totalRounds || result.Reason > 0 ){
                    ...
                    //清理历史数据
                    YxmjUIMgr.getInstance().removeGameInfo();
                }
                else{
                    cc.log("显示结算界面");

                    //显示结算界面
                    var dlgYxmjResult = UIMgr.getInstance().openDlg(ID_DlgmjSettlement);
                    ...
                    dlgYxmjResult.setCallBack(readyCB,shareCB);
                    dlgYxmjResult.show(result,CMD_YXMJ,ID_DlgYxmjCardsInfo);
                }
            };
            if(bZhuaHua) {
    ...
    showCatchFlowerUI:function (resultData,cb) {
        ...
        var Panel_CatchFlower = DlgYxmjMain.Panel_root.getChildByName("Panel_CatchFlower");
    ...
    onBreak:function(){
        ...
        do{
            ...
            //设置自己的第13手牌
            if(currentUser==meChairID && (this.cardData[CMD_YXMJ.MAX_COUNT-1]==100 || this.cardData[CMD_YXMJ.MAX_COUNT-1]==0)){
                for(var i=CMD_YXMJ.MAX_COUNT-1;i>=0;i--)
                {
                    var cardValue = this.cardData[i];
                    if(cardValue != 100&&cardValue != 0){
                        this.cardData[CMD_YXMJ.MAX_COUNT-1] = cardValue;
                        ...
                        this.displayCard(this.cardData[i],i);
                        this.displayCard(this.cardData[CMD_YXMJ.MAX_COUNT-1],CMD_YXMJ.MAX_COUNT-1);
                        cc.log("设置自己的第13手牌"+this.cardData[CMD_YXMJ.MAX_COUNT-1]);
                        break;
                    }
                }
            }
        ...
        do{
            ...
            for(var chairID=0;chairID<CMD_YXMJ.GAME_PLAYER; chairID++){
                for(var j=0;j<cbWeaveCount[chairID];j++)
                {
                    ...
                    var nShowStatus = CMD_YXMJ.WIK_NULL;
                    var cbParam = WeaveItemArray[chairID][j].Param;
                    if(cbParam==CMD_YXMJ.WIK_GANERAL){
                        // 吃 or 碰 这个时候得通过 WeaveKind 来区分
                        nShowStatus = WeaveItemArray[chairID][j].WeaveKind;// 表示 左中右吃 碰
                    }
                    else if(cbParam==CMD_YXMJ.WIK_MING_GANG){
                        nShowStatus = CMD_YXMJ.WIK_PENGGANG; // 碰后再杠
                    }
                    else if(cbParam==CMD_YXMJ.WIK_FANG_GANG){
                        nShowStatus = CMD_YXMJ.WIK_GANG;
                    }
                    else if(cbParam==CMD_YXMJ.WIK_AN_GANG){
                        nShowStatus = CMD_YXMJ.WIK_ANGANG;
                    }
        ...
        do{
            ...
            if(cbActionMask!=0){
                ...
                //打开提示面板
                cc.log("--打开操作提示面板--");
                YxmjUIMgr.getInstance().onOperatorTip();
    ...
    doOutCard: function(selectCard){
        ...
        for(var i=0;i<CMD_YXMJ.MAX_COUNT;i++)
        {
            if(this.imgFUserCard[i]==selectCard){
                ...
                // 判断一下是不是花牌
                if (cardValue>= g_Yxmj_Hua_CardValue[0] &&
                    cardValue<= g_Yxmj_Hua_CardValue[7])
                {
                    ...
                    YxmjGameMsg.getInstance().sendReplaceCard(data);
                    return;
                }
                ...
                if(this.doJustChi==true && game.getOperateUser()==game.getMeChairId()) {
                    ...
                    if(opCodeMask & CMD_YXMJ.WIK_LEFT)
                        lastOpCardValue = opCards[0];
                    else  if(opCodeMask& CMD_YXMJ.WIK_CENTER)
                        lastOpCardValue = opCards[1];
                    else if(opCodeMask& CMD_YXMJ.WIK_RIGHT)
                        lastOpCardValue = opCards[2];
                    
                    if(cardValue == lastOpCardValue) {
                        DlgTip.openSysTip("不能吃出刚吃的牌值");
                       return;
                    }
                }
                ...
                YxmjGameMsg.getInstance().sendOutCard(data);
                SoundMgr.getInstance().playEffect("yxmj_outCard", 0, false);
    ...
    showOutPutCard:function (chairID, cardValue,startPos,endPos) {
        ...
        if (index == 0) {
            ...
            if(startPos==undefined)
                startPos = this.imgFUserCard[CMD_YXMJ.MAX_COUNT-1].getPosition();
        ...
        {
            var startPos = this.imgFOtherCard[chairID][CMD_YXMJ.MAX_COUNT-1].getPosition();
            if (index == 1) {
                x = 1099;
                y = 406.43; // right
        ...
        switch(type){
            ...
            case ccui.Widget.TOUCH_MOVED:{
                ...
                if(this.clickCardBeganPos){
                    ...
                    if(Math.sqrt(disX*disX+disY*disY)>50){
                        ...
                        cc.log("--------------滑动--选牌------20---------");
                        SoundMgr.getInstance().playEffect("xymj_selectCard", 0, false);
                    }
                }
                break;
            }
            case ccui.Widget.TOUCH_ENDED:{
                ...
                if(!this.clickCard){
                    ...
                    sender.setPositionY(20);
                    SoundMgr.getInstance().playEffect("yxmj_selectCard", 0, false);
                }
    ...
    clearOpenCard: function(){
        ...
        for(var actionUser=0; actionUser<CMD_YXMJ.GAME_PLAYER; ++actionUser){
        ...
        if (cardData) {
            for(var i=0; i<cardData.length; ++i) {
                ...
                if(openCard.cardValue==cardValue) {
                    if(CMD_YXMJ.WIK_PENG == openCard.cardMask) {
                        //碰变杠
    ...
    getOpenCard: function(actionUser,provideUser, openCard, index){
        ...
        switch(chairIndex){
            case 0: {//自己
                ...
                if (openCard.cardMask & CMD_YXMJ.WIK_LEFT||
                    openCard.cardMask & CMD_YXMJ.WIK_CENTER||
                    openCard.cardMask & CMD_YXMJ.WIK_RIGHT) { // 吃
                    var childName= "";
                    if(openCard.cardMask & CMD_YXMJ.WIK_LEFT)
                        childName = "Panel_mGangNomal_1";
                    else if(openCard.cardMask & CMD_YXMJ.WIK_CENTER)
                        childName = "Panel_mGangNomal_2";
                    else if(openCard.cardMask & CMD_YXMJ.WIK_RIGHT)
                        childName = "Panel_mGangNomal_3";
                    cardTemp = this.Panel_root.getChildByName(childName).clone();
                }
                else if (openCard.cardMask & CMD_YXMJ.WIK_PENG) {//碰
                    var childName= "";
                ...
                else if (openCard.cardMask & CMD_YXMJ.WIK_GANG) {//明杠
                ...
                else if (openCard.cardMask & CMD_YXMJ.WIK_ANGANG) {//暗杠
                    cardTemp = this.Panel_root.getChildByName("Panel_anGangNomal").clone();
                }
                else if(openCard.cardMask & CMD_YXMJ.WIK_PENGGANG) { // 碰杠 （这种情况目前只出现在断线重连出现）
                    var childName= "";
                    if(direct==1)        childName = "Panel_mGangNomal_3";
                    else if(direct==2)  childName = "Panel_mGangNomal_2";
                    else if(direct==3)  childName = "Panel_mGangNomal_1";
        ...
        else if(openCard.cardMask & CMD_YXMJ.WIK_GANG || openCard.cardMask & CMD_YXMJ.WIK_PENGGANG){ // 杠 or 碰杠
            for(var i=0; i<3; i++){
                var Image_color =cardTemp.getChildByName("Image_card_"+i).getChildByName("Image_color");
                Image_color.loadTexture(this.getFCard(openCard.cardValue), ccui.Widget.PLIST_TEXTURE);
            }
    ...
    showOperator_Chi_Gangs_Choose:function (type,cbfun) {
        ...
        if(type==CMD_YXMJ.WIK_LEFT) {
        	bShowMiddle = false;
            cardsCount = 3;
            var mask = game.getAcionMasks();
            if(mask & CMD_YXMJ.WIK_LEFT){
                childName.push("Panel_mGangNomal_1");
                showCount += 1;
            }
            if(mask & CMD_YXMJ.WIK_CENTER){
            	childName.push("Panel_mGangNomal_2");
            	showCount += 1;
            }
            if(mask & CMD_YXMJ.WIK_RIGHT){
                childName.push("Panel_mGangNomal_3");
                showCount += 1;
            }
        }
        else if(type==CMD_YXMJ.WIK_GANG) {
            cardsCount = 4;
        ...
        for(var i=0;i<showCount; i++)
        {
            ...
            if(type == CMD_YXMJ.WIK_GANG) {
                ...
                if(i<2) {
                    ...
                    this.Chi_Gang_Image_bg.setScaleY(2);
                }
                ...
                cardFrame.setScale(0.8);
                cardFrameW = cardFrame.getContentSize().width;
                x = (btnGuoPos.x -(cardFrameW+5)*i)*0.9 + 20; // 到时候具体再调整
                y = btnGuoPos.y+5;
            ...
            for(var j=0; j<cardsCount; j++){
                ...
                var cardVlue = (type == CMD_YXMJ.WIK_GANG ? cardValues[0]:cardValues[j]);
    ...
    showOperator: function(isShow, arrayShow){
        ...
        for(var index=length-1; index>=0; index--){
            ...
            switch(arrayShow[index]){
                case CMD_YXMJ.WIK_LEFT:{ // 吃 不管那个吃，界面都用这个来代表；
                    obj =  this.Button_chi;
                    break;
                }case CMD_YXMJ.WIK_PENG:{//碰
                    obj =  this.Button_peng;
                    break;
                }case CMD_YXMJ.WIK_GANG:{//杠
                    obj =  this.Button_gang;
                    break;
                }case CMD_YXMJ.WIK_LISTEN:{//听
                    obj =  this.Button_ting;
                    break;
                }case CMD_YXMJ.WIK_CHI_HU://吃胡
                 case CMD_YXMJ.WIK_FANG_PAO:{//放炮
                    obj =  this.Button_hu;
                    break;
                }default:
                    break;
    ...
    onOperatorBtnClick:function(sender, type) {
        if (ccui.Widget.TOUCH_ENDED == type)
        {
            var sendOperateCardFun = function (code,operateCard,codeDetail) {
                ...
                YxmjGameMsg.getInstance().sendOperateCard(data);
            ...
            switch(name){
                case "Button_chi":
                	var a = mask & CMD_YXMJ.WIK_LEFT;
                	var b = (mask & CMD_YXMJ.WIK_CENTER)>>1;
                	var c = (mask & CMD_YXMJ.WIK_RIGHT)>>2;
                	var count = a+b+c;
                	
                	if(1 < count) {
                       // 先选择后发送
                		this.showOperator(false, []);
                        this.showOperator_Chi_Gangs_Choose(CMD_YXMJ.WIK_LEFT,sendOperateCardFun);
                        return;
                    }
                    else {
                        // 直接发送
                        if(mask & CMD_YXMJ.WIK_LEFT) {
                            code = CMD_YXMJ.WIK_LEFT;
                            operateCard[0]=actionCard; operateCard[1]=actionCard+1; operateCard[2]=actionCard+2;
                        } else if(mask & CMD_YXMJ.WIK_CENTER) {
                            code = CMD_YXMJ.WIK_CENTER;
                            operateCard[0]=actionCard-1; operateCard[1]=actionCard; operateCard[2]=actionCard+1;
                        } else if(mask & CMD_YXMJ.WIK_RIGHT) {
                            code = CMD_YXMJ.WIK_RIGHT;
                            operateCard[0]=actionCard-2; operateCard[1]=actionCard-1; operateCard[2]=actionCard;
                        }
                    }
                    break;
                case "Button_peng":
                    code=CMD_YXMJ.WIK_PENG;
                ...
                case "Button_gang":
                    ...
                    if(this.handGangs.length>0) {	// 自己抓牌得到的；
                        ...
                        if (this.handGangs.length>1) {
                        	this.showOperator(false, []);
                        	this.showOperator_Chi_Gangs_Choose(CMD_YXMJ.WIK_GANG,sendOperateCardFun);
                        	return;
                        } else {
                ...
                case "Button_ting":
                    {
                        ...
                        YxmjGameMsg.getInstance().sendTingOperateCard();
                        this.showTingCards();
                        return;
                    }

                case "Button_hu":
                    code=CMD_YXMJ.WIK_CHI_HU;
                    operateCard[0]=actionCard;
                    break;
                case "Button_guo":
                    cade=CMD_YXMJ.WIK_NULL;
                    this.Panel_Chi_Gang_Choose.setVisible(false);
    ...
    operateResult:function() {
        ...
        if(operateUser == mychairid)
        {
            ...
            //碰 or 吃
            if(operateCode & CMD_YXMJ.WIK_PENG ||
                operateCode & CMD_YXMJ.WIK_LEFT||
                operateCode & CMD_YXMJ.WIK_CENTER||
                operateCode & CMD_YXMJ.WIK_RIGHT)
            {
                ...
                // 隐藏相应手牌
                this.cardData[CMD_YXMJ.MAX_COUNT-1]=100;
                for(var i=1;i<3;i++) {
                    for(var j=0;j<CMD_YXMJ.MAX_COUNT;j++) {
                ...
                var cardTemp = 0;
                for(var i=CMD_YXMJ.MAX_COUNT-2;i>=0;i--) {
                ...
                this.updateCard(this.cardData,CMD_YXMJ.MAX_COUNT);
                this.cardData[CMD_YXMJ.MAX_COUNT-1] = cardTemp;
                this.displayCard(cardTemp, CMD_YXMJ.MAX_COUNT-1);

                //显示开牌
               if(operateCode & CMD_YXMJ.WIK_PENG ) {
                   cc.log("---碰----显示开牌");
                   this.setOpenCard(operateUser, provideUser,operateCard[0], CMD_YXMJ.WIK_PENG);
               }
               else {
                   cc.log("---吃----显示开牌");
            ...
            //杠
            if(operateCode & CMD_YXMJ.WIK_GANG)
            {
                // 动画
                ...
                if(provideUser == operateUser){
                    ...
                    for(var i=0;i<this.openFCardData.length;i++){
                        var openCard = this.openFCardData[i];
                        if(openCard.cardMask&CMD_YXMJ.WIK_PENG && openCard.cardValue == operateCard[0]){
                            //明杠(自摸：是抓的牌，非自摸：杠也可能是牌面的牌)
                            bGang = true;
                            this.setOpenCard(mychairid,provideUser, operateCard[0], CMD_YXMJ.WIK_GANG);
                    ...
                    if(!bGang){
                        //自摸暗杠
                        this.setOpenCard(mychairid, provideUser,operateCard[0], CMD_YXMJ.WIK_ANGANG);
                    ...
                    //明杠
                    this.setOpenCard(operateUser, provideUser,operateCard[0], CMD_YXMJ.WIK_GANG);
        ...
        {   //其他玩家
                ...
                operateCode & CMD_YXMJ.WIK_RIGHT)
            {
                ...
                //显示 操作玩家的 第13张手牌
                this.imgFOtherCard[operateUser][CMD_YXMJ.MAX_COUNT-1].setVisible(true);
            ...
            if(operateCode &CMD_YXMJ.WIK_GANG)
            {
                ...
                if(this.openTCardData[operateUser]!=undefined)
                {
                    for(var i=0;i<this.openTCardData[operateUser].length;i++)
                    {
                        ...
                        if(openCard.cardMask&CMD_YXMJ.WIK_PENG && openCard.cardValue == operateCard[0])
                        {
                            ...
                            //移除操作玩家1张牌
                            var tVisibleCard = 0;
                            for(var j=CMD_YXMJ.MAX_COUNT-2; j>=0 ;j--)
                            {
                                if(tVisibleCard==1)
                                    break;
                ...
                if(!bGang)
                {
                    if(provideUser == operateUser)
                    {
                        //自摸暗杠
                        cc.log("+++++++++自摸暗杠+++++++++");
                        this.setOpenCard(operateUser,provideUser,operateCard[0], CMD_YXMJ.WIK_ANGANG);
			}
                    else
                    {
                        cc.log("+++++++++明杠+++++++++");
                    ...
                    //移除操作玩家3张牌
                    var tVisibleCard = 0;
                    for(var k=CMD_YXMJ.MAX_COUNT-2; k>=0 ;k--)
                    {
                        if(tVisibleCard==3)
                            break;
        ...
        //播放操作 声音
        cc.log("---播放操作 声音---");
        if(operateCode & CMD_YXMJ.WIK_LEFT||
            operateCode & CMD_YXMJ.WIK_CENTER||
            operateCode & CMD_YXMJ.WIK_RIGHT){
            SoundMgr.getInstance().playEffect("yxmj_"+strSex+"_chi", sex, false);
        }
        else if(operateCode & CMD_YXMJ.WIK_PENG){
            SoundMgr.getInstance().playEffect("yxmj_"+strSex+"_peng", sex, false);
        }
        else if(operateCode & CMD_YXMJ.WIK_GANG){
            SoundMgr.getInstance().playEffect("yxmj_"+strSex+"_gang", sex, false);
        }
        else if(operateCode & CMD_YXMJ.WIK_CHI_HU){
            SoundMgr.getInstance().playEffect("yxmj_"+strSex+"_chi_hu", sex, false);
        }
    },
