...
var YxmjGame = cc.Class.extend({
	...
	reset: function(){
		...
		this.gameStatus = CMD_YXMJ.GS_MJ_FREE;
		this.cbMagicIndex = CMD_YXMJ.MAX_INDEX;
		...
		this.lSiceCount = 0;
		//玩家牌型初始化
		this.playerCards = new Array(CMD_YXMJ.GAME_PLAYER);
		for(var i=0;i<CMD_YXMJ.GAME_PLAYER;i++)
		{
			this.playerCards[i] = new Array(CMD_YXMJ.MAX_COUNT);
		...
		this.wHeapHead = 0;
		this.wHeapTail = 0;		
		this.cbHeapCardInfo = new Array(CMD_YXMJ.GAME_PLAYER);
		for(var i=0;i<CMD_YXMJ.GAME_PLAYER;i++)
		{
		...
		// 动作数据
		this.cbActionCard = CMD_YXMJ.WIK_NULL;
		this.cbActionMask = CMD_YXMJ.WIK_NULL;
	...
	/*getMeUserInf:function(){
		...
		if(this.meChairId>=CMD_YXMJ.GAME_PLAYER) return null;
	...
	setHisData:function(chairId,lTurnScore,lCollectScore){
		...
		if(chairId>=CMD_YXMJ.GAME_PLAYER) return;
		var bFind=false;
	...
	/*	switchViewChairID:function(wChairID){
		...
		// var wChairCount = CMD_YXMJ.GAME_PLAYER;
		// var wViewChairID =
	...
	getMeCards:function(){
		...
		var meCards=new Array(CMD_YXMJ.MAX_COUNT);
		for(var j=0;j<CMD_YXMJ.MAX_COUNT;j++)
	...
	getOtherCards:function(){
		var otherCards=new Array(CMD_YXMJ.MAX_COUNT);
