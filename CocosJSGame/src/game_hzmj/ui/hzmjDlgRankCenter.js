...
cc.log("--------ID_DlgHzmjRankCenter = "  + ID_DlgHzmjRankCenter);
DLG_CREATOR[ID_DlgHzmjRankCenter] = function() {
	return new DlgHzmjRankCenter();
};

var DlgHzmjRankCenter = DlgBase.extend({
	//成员函数
	ctor: function(){	
	...
	init: function() {		
		...
		var json = ccs.load(res.hzmjDlgRankCenter_json);
	...
	show: function(){
		...
		for(var i=0;i<CMD_HZMJ.GAME_PLAYER;i++)
		{
			...
			var Image_nomal = this.Panel_userInfo.getChildByName("Node_userInfo_"+i).getChildByName("Image_nomal");
			Image_nomal.getChildByName("Text_nickName").string = player.getNickName();
			Image_nomal.getChildByName("Text_gold").string = ""+player.getScore();
			cc.log("玩家信息-"+player.getNickName()+"-"+player.getScore());
		...
		//分享按钮
		var Button_xuanYao = this.setBtnCallBack(this.Panel_root, "Button_xuanYao", this.onClickEvent);

		//返回按钮
		var Button_return = this.setBtnCallBack(this.Panel_root, "Button_return", this.onClickEvent);
		
		//续费按钮
		var Button_continue = this.setBtnCallBack(this.Panel_root, "Button_continue", this.onClickEvent);
		if(game.getTableOwnerUserID()!=player.getUserId()){
		...
		for(var i=0; i<totalRounds; ++i){
			...
			if(i%2==1){
				Panel_rankItem.getChildByName("Image_bg").setVisible(false);
			}
		...
		{//总积分
			...
			if(i==g_objHero.getChairID()){
				if(totalScore<=0)
					SoundMgr.getInstance().playEffect("hzmj_rankLose", 0, false);
				else
					SoundMgr.getInstance().playEffect("hzmj_rankWin", 0, false);
			}
		}
	},
	...
	onClickEvent: function(sender, type){
		if (ccui.Widget.TOUCH_ENDED == type){	
			...
			case "Button_return":
				cc.log("++++++战绩中心界面----返回++++++");
				//关闭战绩中心
				UIMgr.getInstance().closeDlg(ID_DlgHzmjRankCenter);
