DLG_CREATOR[ID_DlgSSSBegin] = function() {
	return new DlgSSSBeginScene();
};

var DlgSSSBeginScene = DlgBase.extend({
	ctor: function () {
	...
	init: function(){
		...
		var json = ccs.load(res.sssdlgBegin_json);
        ...
        this.Button_cancel = this.Panel_NPlayer.getChildByName('Button_cancel');
		...
		if (ccui.Widget.TOUCH_ENDED == type) {
			...
			case "Button_begin":	
				if (ccui.Widget.TOUCH_ENDED == type) {
					...
					var PlayerCN = UIMgr.getInstance().getDlg(ID_DlgSSSPlayerData);
