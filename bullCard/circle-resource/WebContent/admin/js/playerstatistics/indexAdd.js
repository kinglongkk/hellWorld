 ...
 * echart玩家新增数
 */
function baiduEchartAdd(){
    var myChart = echarts.init(document.getElementById('addLinesLabels'));
    myChart.showLoading({
        text: '正在努力的读取数据中...'
    });
    ...
    var rootUrl = root+"/playerSummery/indexMultipleAddAjax.html?year="+year+"&month="+month+"&username="+agent;
    $.ajax({
        type:"post",
        url:  rootUrl,
        async : true
...
//     $("#sliders").hide();
//     // $("#columnRotatedLabels").css("height",    $(document.body).height()-200);
//     var dataStr = $("#dataStr").val().split(",");
//     var dataInt=[];
//     $.ajax({
//         type:"post",
