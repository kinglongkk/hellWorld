...
<div class="row">
    <div class="position-wrap clearfix">
        ...
        <span>${views.sysResource['账号管理']}</span><span>/</span><span>${views.sysResource['玩家管理']}</span>
                    ...
                    <c:choose>
                        <c:when test="${command.result.accountfreeze}">
                            <soul:button target="${root}/share/account/toCancelAccountFreeze.html?search.id=${command.result.id}&sign=player"
                                         text="${views.column['player.cancelAccountFreeze']}"
                                         opType="dialog"
                                         cssClass="btn btn-outline btn-filter btn-sm${option_btn_disabled ?' disabled':''}"
                                         callback="reloadViewWithoutReturnValue" confirm="${views.column['player.cancelAccountFreezeOk']}">
                                ${views.column['player.cancelAccountFreeze']}
                            </soul:button>
                        </c:when>
                        <c:otherwise>
                            <soul:button target="${root}/share/account/freezeAccount.html?result.id=${command.result.id}"
                                        text="${views.column['player.freezeAccount']}"
                                        opType="dialog"
                                        cssClass="btn btn-outline btn-filter btn-sm${option_btn_disabled ?' disabled':''}"
                                        callback="reloadViewWithoutReturnValue">
                                ${views.column['player.freezeAccount']}
                            </soul:button>
                        </c:otherwise>
                    </c:choose>
                    ...
                    <c:choose>
                        <c:when test="${command.result.status == '2'}">
                            <soul:button
                                    ...
                                    text="${views.column['player.disabled']}"
                                    name="accountDisabled"
                                    opType="ajax"
                                    confirm="${views.column['player.confirm.cancelstop']}"
                                    callback="reloadViewWithoutReturnValue">
                                ${views.column['player.disabled']}
                            </soul:button>
                        </c:when>
                        <c:otherwise>
                            <soul:button
                                    target="${root}/share/account/disabledAccount.html?result.id=${command.result.id}"
                                    text="${messages['player.accountDisabled']}"
                                    opType="dialog"
                                    cssClass="btn btn-outline btn-filter btn-sm">
                                ${views.column['player.accountDisabled']}
                            </soul:button>
                        </c:otherwise>
                    </c:choose>

                    <%--<soul:button title="${views.column['player.resetLoginPwd']}" target="${root}/player/resetPwd/index.html?resetType=loginPwd&userId=${command.result.id}" cssClass="${option_btn_disabled ?'disabled':''} btn btn-outline btn-filter btn-sm" text="" opType="dialog" tag="button">--%>
                        <%--${views.column['player.resetLoginPwd']}--%>
                    <%--</soul:button>--%>

                    <a href="/report/fundsLog/list.html?search.username=${command.result.username}" class="btn btn-outline btn-filter btn-sm areport" nav-target="mainFrame">${views.column['player.fundsLog']}</a>
                </div>
            </div>
            <div class="">
                    ...
                    <div class="">
                        <div class="panel-options">
                            <ul class="nav nav-tabs p-l-sm p-r-sm">
                                <li class="active">
                                    <a data-toggle="tab" href="#playerInfo${command.result.id}" aria-expanded="true" data-load="1">${views.column['player.playerDetail']}</a>
                                </li>
                                <li>
                                    <a data-toggle="tab" href="#remark${command.result.id}" aria-expanded="false" data-href="${root}/playerRemark/remark.html?search.entityUserId=${command.result.id}&search.operatorId=${command.result.id}">${views.column['player.playerRemark']}</a>
                                </li>
                                <li>
                                    <a data-toggle="tab" href="#journal${command.result.id}" aria-expanded="false" data-href="${root}/playerLog/view/journal.html?search.entityId=${command.result.id}">${views.column['player.playerLog']}</a>
                                </li>
                            </ul>
                        </div>
                    </div>
