...
<form:form action="${root}/player/playerView.html?search.id=${command.result.id}&ajax=true" method="post">
    ...
    <table class="table  dataTable">
        <tbody>
        <tr>
            <td class="bg-tbcolor">${views.column['role.player.belongProxy']}:${command.result.agentName}</td>
            <td class="bg-tbcolor">${views.column['role.player.playerAccount']}:${command.result.username}</td>
            <td class="bg-tbcolor">${views.column['role.player.nickName']}:${command.result.nickname}</td>
        </tr>
        <tr>
            ...
            <td class="bg-tbcolor">${views.column['role.player.lastLoginTime']}:${soulFn:formatDateTz(command.result.lastLoginTime, DateFormat.DAY_SECOND,timeZone)}</td>
        </tr>
