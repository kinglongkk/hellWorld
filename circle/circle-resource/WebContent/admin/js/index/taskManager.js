...
define(['common/BasePage'], function (BasePage) {
    ...
    return BasePage.extend({
        ...
        init: function () {
            ...
            _this.syncUserTime()
            //是不是使用临时域名
            if($("#domainTemp").val()=='true'){
                //主域名和管理中心域名都是首次添加
                window.top.topPage.doDialog({page:this},{text:window.top.message.common['msg'],target: root + "/sysDomainOperator/toDefaultDomain.html",callback:""});
            }
        },
        /**
        ...
        userTime: function () {
            var _this=this;
            if(_this.dateTimeFromat!=undefined && $(".clock-show").attr("time")) {
                var date = new Date();
